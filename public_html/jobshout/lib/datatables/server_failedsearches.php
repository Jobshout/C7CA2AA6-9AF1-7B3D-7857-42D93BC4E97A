...
require_once("lib.inc.php");
	...
	$set_session= set_session_values('failedsearches',$sSearch,$iDisplayStart,$iDisplayLength);
	 ...
	 */
	 $aColumns = array('id', 'srch_term', 'ip', 'browser', 'date_time');
	 
	$aColumns1 = array('id', 'srch_term', 'ip', 'browser', 'date_time');
	
	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "id";
	
	/* DB table to use */
	$sTable = "FailedWebSearches";
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		{
				if($aColumns1[$i]=='date_time'){
				$row[] = date('d M Y H:i:s',$aRow[ $aColumns1[$i] ]);
				}
				else{
				/* General output */
