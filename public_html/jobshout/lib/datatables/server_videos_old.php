...
require_once("lib.inc.php");
	 ...
	 */
	 $aColumns = array('sites.Name as SiteName', 'videos.title', 'videos.video_sort_order', 'videos.published_timestamp', 'videos.modified', 'videos.active', 'videos.uuid');
	 
	$aColumns1 = array('SiteName', 'title', 'video_sort_order', 'published_timestamp', 'modified', 'active', 'uuid');
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns1) ; $i++ )
		{
			...
			$sql_pic_cat=mysql_query("select * from  videocategories where Video_GUID='".$aRow[ $aColumns1[$i] ]."'");
